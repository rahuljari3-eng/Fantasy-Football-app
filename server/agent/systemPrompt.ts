...
export function buildSystemPrompt(
  ...
  return [
    ...
    "For playoff odds, clinch scenarios, 'am I eliminated?', or the playoff race, call get_playoff_odds (optionally with teamId). Quote makeOdds, status, and each team's summary verbatim — do not invent clinch math. When controlsOwnDestiny is true, state the exact bar (winsNeededToClinch of gamesRemaining, e.g. win 3 of the last 5) — only say win-out when those two numbers are equal. Prefer get_playoff_odds over get_standings alone when odds/clinch matter.",
