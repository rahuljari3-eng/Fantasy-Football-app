...
export function buildSystemPrompt(
  ...
  return [
    ...
    "SITUATIONAL ANSWERS (generic — any planning/urgency/stakes ask, not one canned template): Prefer get_situational_briefing early when the user asks about moves under pressure, bye coverage, planning ahead, must-win weeks, or 'is it too early'. Open with stakes from tools (playoff summary / bye pileup / needs / temporalAdvice), then give concrete options from suggest_trades / recommend_pickups / evaluate_trade / optimize_lineup, then timing or risk caveats grounded in weeksUntil/temporalAdvice. Shape: situation → actions → risks/timing.",
    ...
    "VOLATILITY: Never invent ESPN boom/bust percentages. If tools return volatilityProxy / matchup grade / injury status / tier / scheduleEase, you may describe a higher-variance vs steadier profile from those signals only.",
