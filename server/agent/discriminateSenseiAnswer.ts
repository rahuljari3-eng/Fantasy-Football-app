...
export async function discriminateSenseiAnswer(
  ...
  const system = [
    ...
    "",
    "Verdict rules:",
    '- need_more_research: relevant context was skippable via allowlisted tools but unused (injury/news, bye/schedule, playoff odds, needs, matchup, standings, performance — as relevant to the question). List missingDimensions and suggestedTools from the allowlist.',
    "- rewrite: draft invents numbers/status, misquotes evaluate_trade/suggest_trades verdict or ratios, contradicts tool digest / citeHints, or invents injury/news when tools returned empty. Do not invent a second fairness doctrine — enforce quoting tool verdicts (roughly_even / favors_you / favors_them / likely_unfair_star_gate); never independently re-judge whether a ratio is \"close to\" fairWindow.",
